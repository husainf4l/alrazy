...
const Sidebar = ({ activeItem, onItemClick }: SidebarProps) => {
  ...
  const menuItems: MenuItem[] = [
    { id: 'dashboard', icon: 'dashboard', label: 'Security Dashboard', badge: null, path: '/dashboard' },
    { id: 'analytics', icon: 'analytics', label: 'Threat Analytics', badge: null, path: '/dashboard/analytics' },
    { id: 'users', icon: 'users', label: 'User Access', badge: '12', path: '/dashboard/users' },
    { id: 'projects', icon: 'projects', label: 'Security Policies', badge: '3', path: '/dashboard/projects' },
    { id: 'messages', icon: 'messages', label: 'Security Alerts', badge: '5', path: '/dashboard/messages' },
    { id: 'calendar', icon: 'calendar', label: 'Incident Timeline', badge: null, path: '/dashboard/calendar' },
    { id: 'settings', icon: 'settings', label: 'Security Settings', badge: null, path: '/dashboard/settings' },
  ];
  ...
  return (
    <div className="w-14 bg-white/95 backdrop-blur-xl shadow-2xl border-r border-gray-200/50 flex flex-col fixed left-0 top-0 h-screen z-40">
      ...
      <div className="flex flex-col h-full">
        {/* Logo */}          <div className="h-16 flex items-center justify-center border-b border-gray-100/50">
          ...
          <div className="space-y-2">
            {menuItems.slice(0, -1).map((item) => (
              ...
              >
                ...
                >
                  <Icon name={item.icon} className={`transition-all duration-300 ${
        ...
        <div className="pb-5">
          ...
          >
            <button
              onClick={() => handleItemClick(menuItems.find(item => item.id === 'settings')!)}
              className={`relative flex items-center justify-center w-10 h-10 mx-auto rounded-xl transition-all duration-300 transform hover:scale-105 ${
                activeItem === 'settings'
                  ? 'bg-gradient-to-br from-red-50 to-orange-50 text-red-600 shadow-lg shadow-red-100/50 scale-105'
                  : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 hover:shadow-md'
            ...
            >
              <Icon name="settings" className={`transition-all duration-300 ${
                activeItem === 'settings' ? 'w-5 h-5 animate-spin-slow' : 'w-4 h-4'
              }`} />
