...
const CameraStreamGrid: React.FC<CameraStreamGridProps> = ({ 
  ...
  // Real-time camera streaming implementation
  useEffect(() => {
    if (isLoading) return;
    ...
    const setupCameraStreaming = () => {
      cameras.forEach(async (camera) => {
        ...
        try {
          console.log(`🎥 Setting up streaming for Camera ${camera.id}`);
          
          // First try MJPEG streaming (continuous video)
          const mjpegUrl = `/api/camera-stream?cameraId=${camera.id}`;
          
          // Test if MJPEG is available
          const streamTest = await fetch(mjpegUrl, { method: 'HEAD' }).catch(() => null);
          
          if (streamTest && streamTest.ok) {
            console.log(`✅ Using MJPEG stream for Camera ${camera.id}`);
            setFrameData(prev => ({
              ...prev,
              [camera.id]: `${mjpegUrl}&t=${Date.now()}`
            }));
          } else {
            console.log(`📸 Fallback to snapshots for Camera ${camera.id}`);
            
            // Fallback to periodic snapshots with faster refresh rate based on quality
            const updateSnapshot = async () => {
              if (!streamConnections[camera.id]) return;
              
              try {
                const imageUrl = `/api/camera-snapshot?cameraId=${camera.id}&t=${Date.now()}`;
                ...
                img.onload = () => {
                  ...
                  setCameras(prev => prev.map(cam => 
                    cam.id === camera.id 
                      ? { ...cam, status: 'connected', lastConnected: new Date() }
                      : cam
                  ));
                };
                
                img.onerror = () => {
                  console.warn(`Failed to load snapshot for camera ${camera.id}`);
                  
                  // Update camera status to error and create fallback image
                  setCameras(prev => prev.map(cam => 
                    cam.id === camera.id 
                      ? { ...cam, status: 'error', errorMessage: 'Snapshot failed' }
                      : cam
                  ));
                  
                  // Create fallback canvas image with RTSP info
                  ...
                  if (ctx) {
                    ...
                    ctx.fillText('RTSP Stream Available', 320, 170);
                    
                    ctx.font = '14px system-ui';
                    ctx.fillStyle = '#9ca3af';
                    ctx.fillText('HTTP Preview Not Available', 320, 190);
                    
                    ctx.font = '12px system-ui';
                    ctx.fillStyle = '#6b7280';
                    ctx.fillText(`${camera.url}`, 320, 210);
                    ctx.fillText(`${new Date().toLocaleTimeString()}`, 320, 230);
                    
                    // Add pulsing dot for RTSP indication
                    const time = Date.now() / 1000;
                    ctx.globalAlpha = 0.5 + 0.5 * Math.sin(time * 2);
                    ctx.fillStyle = '#ef4444';
                    ctx.beginPath();
                    ctx.arc(320, 250, 4, 0, 2 * Math.PI);
                    ctx.fill();
                    ctx.globalAlpha = 1;
                ...
                console.error(`Error updating snapshot for camera ${camera.id}:`, error);
              }
            };
            ...
            // Set refresh rate based on quality
            const refreshRate = streamingQuality === 'high' ? 500 :   // 2 FPS
                              streamingQuality === 'medium' ? 1000 : // 1 FPS  
                              2000; // 0.5 FPS
                              
            const interval = setInterval(updateSnapshot, refreshRate);
            
            // Store interval for cleanup
            (window as any)[`cameraInterval_${camera.id}`] = interval;
          }
          
        } catch (error) {
          console.error(`Error setting up streaming for camera ${camera.id}:`, error);
          setCameras(prev => prev.map(cam => 
            cam.id === camera.id 
              ? { ...cam, status: 'error', errorMessage: error instanceof Error ? error.message : 'Connection failed' }
              : cam
          ));
        }
      });
    ...
    // Setup streaming for all cameras
    ...
    return () => {
      ...
      Object.keys(streamConnections).forEach(cameraId => {
        ...
        const interval = (window as any)[`cameraInterval_${cameraId}`];
        if (interval) {
          clearInterval(interval);
          delete (window as any)[`cameraInterval_${cameraId}`];
        }
      });
    };
