    ...
    const setupCameraStreaming = () => {
      cameras.forEach(async (camera) => {
        ...
        try {
          ...
          console.log(`📡 RTSP URL: ${camera.url.replace(cameraConfig.password, '***')}`);
          ...
          const proxyTest = await fetch(rtspProxyUrl, { 
            ...
            console.log(`⚠️ RTSP proxy test failed for Camera ${camera.id}:`, err.message);
            return null;
          });
          
          if (proxyTest && proxyTest.ok) {
            console.log(`✅ Using RTSP-to-MJPEG streaming for Camera ${camera.id}`);
            ...
            const updateSnapshot = async () => {
              ...
              try {
                const snapshotUrl = `/api/camera-snapshot?cameraId=${camera.id}&t=${Date.now()}`;
                ...
                img.onload = () => {
                  ...
                  setCameras(prev => prev.map(cam => 
                    cam.id === camera.id 
                      ? { ...cam, status: 'connected', streamType: 'HTTP-Snapshot' }
                      : cam
                  ));
                ...
                img.onerror = () => {
                  ...
                  if (ctx) {
                    ...
                    ctx.fillText(`Channel: ${camera.id} • ${camera.resolution} • ${camera.fps}fps`, 320, 220);
                  ...
                  setCameras(prev => prev.map(cam => 
                    cam.id === camera.id 
                      ? { ...cam, status: 'connected', streamType: 'RTSP-Only' }
                      : cam
                  ));
                ...
                setCameras(prev => prev.map(cam => 
                  cam.id === camera.id 
                    ? { ...cam, status: 'error', errorMessage: error instanceof Error ? error.message : 'Unknown error' }
                    : cam
                ));
              }
            };
            ...
            const updateInterval = streamingQuality === 'high' ? 500 : 
                                 streamingQuality === 'medium' ? 1000 : 2000;
          ...
          setCameras(prev => prev.map(cam => 
            cam.id === camera.id 
              ? { ...cam, status: 'error', errorMessage: error instanceof Error ? error.message : 'Setup failed' }
              : cam
          ));
        }
      });
    ...
    return (
      <div className="flex items-center justify-center h-96 bg-white rounded-2xl border border-gray-100">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Connecting to RTSP camera streams...</p>
          <p className="text-sm text-gray-500 mt-2">IP: {cameraConfig.base_ip}:{cameraConfig.rtsp_port}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="p-4 bg-gray-50 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h3 className="font-semibold text-gray-900">RTSP Live Camera Feeds</h3>
              <span className="text-sm text-gray-500">• {cameras.filter(c => c.status === 'connected').length}/4 active</span>
                ...
                <div className="w-full h-full bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center relative">
                  {frameData[selectedCamera] ? (
                    ...
                    <div className="text-center text-white">
                      ...
                      <p className="text-sm opacity-50">RTSP Stream Loading...</p>
                    </div>
                  )}
                  ...
                  <div className="absolute top-4 left-4 flex items-center space-x-2">
                    ...
                    <span className="text-white text-sm font-semibold bg-black bg-opacity-50 px-3 py-1 rounded-full backdrop-blur-sm">
                      {selectedCameraData.streamType === 'RTSP-MJPEG' ? 'RTSP LIVE' : 'LIVE'}
                    </span>
                  </div>
                  ...
                  <div className="absolute top-4 right-4 bg-black bg-opacity-50 text-white px-3 py-2 rounded-lg backdrop-blur-sm">
                    ...
                    {selectedCameraData.streamType && (
                      <div className="text-xs opacity-75">{selectedCameraData.streamType}</div>
                    )}
                  </div>
                  ...
                  <div className="absolute bottom-4 left-4 bg-black bg-opacity-50 text-white px-3 py-1 rounded-lg backdrop-blur-sm">
                    <div className="text-xs">
                      ...
                      {selectedCameraData.streamType === 'RTSP-MJPEG' && ' • H.264'}
                    </div>
                  </div>
                  ...
                  {connectionTest[selectedCamera] && (
                    <div className="absolute bottom-4 right-4 bg-black bg-opacity-50 text-white px-3 py-1 rounded-lg backdrop-blur-sm">
                      <div className="text-xs opacity-75">{connectionTest[selectedCamera]}</div>
                    </div>
                  )}
                </div>
              </>
                ...
                <div className="w-full h-full bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center relative">
                  ...
                  <div className="absolute top-2 left-2">
                    <div className={`w-2 h-2 rounded-full shadow-lg ${
                      ...
                      camera.status === 'loading' ? 'bg-yellow-400 animate-pulse' :
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`w-3 h-3 rounded-full ${
                      selectedCameraData.status === 'connected' ? 'bg-green-500 animate-pulse' : 'bg-red-500'
                    }`}></div>
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        ...
                        {selectedCameraData.streamType && ` • ${selectedCameraData.streamType}`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs font-mono text-gray-700">
                        {selectedCameraData.url.replace(cameraConfig.password, '***')}
                      </div>
                    </div>
                    ...
                    <div className="text-center">
                      ...
                      <div className={`text-xs font-medium ${
                        selectedCameraData.status === 'connected' ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {selectedCameraData.status === 'connected' ? '● STREAMING' : '● OFFLINE'}
                      </div>
                    </div>
                  </div>
                </div>
