...
export default function VideoStreamPage() {
  ...
  const connectToStream = async (sessionId: string) => {
    ...
    try {
      ...
      if (offerResponse.status === 410) {
        // Connection was closed, try to recreate
        console.log('Connection was closed, attempting to recreate...');
        setConnectionStatus('Recreating session...');
        
        const recreateResponse = await fetch(`${apiUrl}/api/webrtc/stream/${sessionId}/recreate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' }
        });
        ...
        if (recreateResponse.ok) {
          ...
          const newSessionId = recreateData.new_session_id;
          console.log(`Session recreated: ${sessionId} -> ${newSessionId}`);
          
          // Update selected stream to new session ID
          setSelectedStream(newSessionId);
          return; // Exit and let useEffect handle the new connection
        } else {
          throw new Error('Failed to recreate session after connection was closed');
        }
      }
