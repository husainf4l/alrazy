...
class SecurityOrchestrator:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if llm_config:
                # Convert LLMConfig object to dict if needed
                if hasattr(llm_config, 'enabled'):
                    ...
                    initialize_llm_analyzer(llm_dict)
                else:
                    initialize_llm_analyzer(llm_config)
                self.system_status["llm_analysis"] = True
                logger.info("✓ LLM analysis service initialized")
            
            # Initialize recording service
            recording_config = None
            if hasattr(self.config, 'recording_config'):
                recording_config = self.config.recording_config
            elif isinstance(self.config, dict):
                recording_config = self.config.get("recording_config", {})
            else:
                recording_config = {}
            
            if hasattr(recording_config, 'recordings_dir'):
                # It's a RecordingConfig object
                initialize_recording_service(
                    recordings_dir=recording_config.recordings_dir,
                    buffer_duration=recording_config.buffer_duration
                )
            else:
                # It's a dictionary
                initialize_recording_service(
            ...
            # Handle WebhookConfig object vs dictionary
            if hasattr(webhook_config, 'pharmacy_name'):
                # It's a WebhookConfig object
                initialize_webhook_service(
                    pharmacy_name=webhook_config.pharmacy_name
                )
            else:
                # It's a dictionary
                initialize_webhook_service(
