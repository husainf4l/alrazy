...
class FaceRecognitionService:
    ...
    def _validate_real_face(self, image: np.ndarray) -> bool:
        ...
        try:
            face_objs = DeepFace.extract_faces(
                ...
                align=True,  
                expand_percentage=35  # Expand by 35% to capture entire head including hair, ears, forehead, chin
            )
    ...
    def recognize_faces(self, image: np.ndarray) -> Dict:
        ...
        """
        try:
            # Use high-quality face extraction with expanded area to capture full facial dimensions
            ...
            try:
                for i, face_data in enumerate(results):
                    ...
                    # Ensure face is in proper format (0-255 uint8)
                    if face_array.max() <= 1.0:
                        face_array = (face_array * 255).astype(np.uint8)
                    else:
                        face_array = face_array.astype(np.uint8)
                    
                    # Validate face dimensions - ArcFace requires minimum 112x112
                    # With 35% expansion, we capture the entire head region
                    if face_array.shape[0] < 80 or face_array.shape[1] < 80:
                        print(f"Face {i+1} too small after extraction: {face_array.shape}, skipping")
                        continue
                    
                    # Resize if needed to ensure optimal quality for ArcFace (target: 112x112 minimum)
                    ...
                    if h < 112 or w < 112:
                        ...
                        print(f"Face {i+1} resized from {w}x{h} to {new_w}x{new_h} for optimal quality")
                    
                    confidence = face_data.get("confidence", 0)
                    print(f"Processing validated face {i+1} with confidence: {confidence:.3f}, dimensions: {face_array.shape}")
